...
def set_save_path(save_path, remove=True):
    ...
    writer = SummaryWriter(os.path.join(save_path, 'tensorboard'))   # make filename for the tensorboard
    return log, writer
...
def radial_encode(delta_X, encoord_dim, angle_num):
    ...
    for i in range(angle_num):
        rotated_coord[:, :, i*2] = torch.cos(rotate_angle[:, :, i]) * dx1 - torch.sin(rotate_angle[:, :, i]) * dx2
        rotated_coord[:, :, i*2+1] = torch.sin(rotate_angle[:, :, i]) * dx1 + torch.cos(rotate_angle[:, :, i]) * dx2

    # encode the rotated coordinates with sin(), cos() functions
    coord = torch.zeros([*rotated_coord.shape[:-1], 2*encoord_dim*(2*rotate_angle.shape[-1])])
    for i in range(angle_num):
        encode_dx = encode_coordnate(rotated_coord[:, :, i*2:(i+1)*2], encoord_dim)
